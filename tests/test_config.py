"""测试配置加载"""
import os
from utils.config import load_config, get_db_path


def test_load_default_config():
    """测试加载默认配置"""
    config = load_config('config.yaml')
    assert 'database' in config
    assert config['database']['path'] == 'tasks.db'


def test_load_test_config():
    """测试加载测试配置"""
    config = load_config('config.test.yaml')
    assert 'database' in config
    assert config['database']['path'] == 'test_tasks.db'


def test_get_db_path_production():
    """测试生产环境数据库路径"""
    db_path = get_db_path('config.yaml')
    assert db_path.endswith('tasks.db')
    assert os.path.isabs(db_path)  # 应该是绝对路径


def test_get_db_path_test():
    """测试环境数据库路径"""
    db_path = get_db_path('config.test.yaml')
    assert db_path.endswith('test_tasks.db')
    assert os.path.isabs(db_path)


def test_auto_use_test_config_in_pytest():
    """pytest 运行时应该自动使用测试配置"""
    # 这个测试本身在 pytest 中运行，所以会自动使用测试配置
    db_path = get_db_path()
    # 在 pytest 环境下应该使用测试数据库
    assert db_path.endswith('test_tasks.db')


def test_custom_config_via_env(monkeypatch, tmp_path):
    """测试通过环境变量指定配置文件"""
    # 创建自定义配置文件
    custom_config = tmp_path / "custom.yaml"
    custom_config.write_text("""
database:
  path: "custom.db"
""")
    
    monkeypatch.setenv('ALFRED_CONFIG', str(custom_config))
    config = load_config()
    
    assert config['database']['path'] == 'custom.db'
